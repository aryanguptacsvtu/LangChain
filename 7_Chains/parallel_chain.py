...
model1 = ChatGroq(model="llama-3.1-8b-instant", temperature=0.5)
model2 = ChatGroq(model="gemma2-9b-it", temperature=0.5)
parser = StrOutputParser()

prompt1 = PromptTemplate(template="Generate short and simple notes from the following text \n{text}",
                         input_variables=["text"])

prompt2 = PromptTemplate(template="Generate 5 short question answers for the following text \n{text}",
                         input_variables=["text"])
